...
fn main() {
  // &str 用「栈」存储
  let foo = "Hello";    // 字符串 "Hello" 赋值给变量 foo
  let bar = foo;        // 将变量 foo 复制并赋值给 bar
  println!("{}", foo);  // 这样做没有问题！
}
```

可以发现，当我们直接使用 `&str` 存储字符串时，Rust 是将前一个变量 `foo` 的值**直接复制**给后一个变量 `bar`，前一个变量 `foo` 并没有变化。此时我们访问前一个变量 `foo` 没有任何问题。
...
fn main() {
  // String 用「堆」存储
  let foo = String::from("Hello again"); // String 变量 "Hello again" 赋值给 foo
  let bar = foo;                         // 将 foo 变量中的内容**移动**到 bar 中
  println!("{}, {}", foo, bar);          // 出错了！foo 中内容被 moved，无法被 reference
}
```

此时，编译器会报出 `error[E0382]: borrow of moved value: 'foo'` 的错误。可以发现，当我们使用 String 存储字符串时，Rust 不会将变量的值「复制」，而是会将变量「移动」到目标变量中。这种情况下，Rust 会认为前一个变量 `foo` 已经不再有效（no longer valid），因此，当我们在之后试图访问 `foo` 时，Rust 就会报出这一错误。

![](https://i.loli.net/2020/01/25/RyHMb8QnfKir5hC.png)

为什么 Rust 在使用「堆」进行动态内存分配时，会 move 而不 copy 呢？一方面是因为 copy 的消耗是比 move 大得多的；另一方面，Rust 这一设计恰好帮助我们避免了 C 语言中非常可能遇到的一种内存泄露的问题：**double free 异常**。
